...
#        $mail->Password   = 'abc999cba';
        ...
        try {
    		foreach (explode(",", $email) as $email)
                ...
                $mail->SetFrom("ki0001026@gmail.com", "Кон�?тр�?ктор �?мперий");
                $mail->Subject = "Кон�?тр�?ктор �?мперий";
...
#			$balance = (float) @$out[1][0] + 0;
				if(preg_match_all("/(\-)?\d+(\.|,)?(\d+)?/", @$out[1][0], $out2))
				{
				$balance = isset($out2[0][0])? (float) str_replace(",", ".", $out2[0][0]): 0;
				if ($balance > 0 && (strpos($out[1][0], "Задолженность") !== false || strpos($out[1][0], "задолженноcть") !== false || strpos($out[1][0], "Минус:") !== false))
			        $balance = 0 - $balance;
				}
				else 
				{echo "cannot get ballance";
				$balance=-0.01;
				}
...
echo "update callers set {$row['mark']}  active = '{$active}', balance = {$balance}, updateactive='".date("Y-m-d H:i:s")."' where id = {$row['id']}\n";

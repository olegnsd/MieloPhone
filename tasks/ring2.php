...
<?php 
    ...
    if (!(!empty($process) && count($process)>0)) die ("no active tasks\n");
    ...
    if ($process['sms_enable'] == 1) {
        $sms = new Transport();
	 fwrite($fp, date("H:i:s\t")."!!!! Отправка sms ".json_encode($process)."\n");
    ...
    array("ssl" => array("verify_peer" => false,"verify_peer_name" => false,"allow_self_signed" => true)));                     
        try {   $mail->AddAddress($contact['email']);
                ...
                fwrite($fp, date("d-m H:i:s\t")."!!!! Отправка email ".json_encode($process)."\n");
...
{		$query = http_build_query(array('name' => $contact['name'],'email' => $contact['email'],'phone' => $contact['phone'],'date'  => $contact['datering']));
                ...
                $output = curl_exec($ch);
                curl_close($ch);
                fwrite($fp, date("H:i:s\t")."!!!! Дёргаем курл \n".json_encode($process)."\n");
                if ($output == "Call not found")  { return;}
}
	fclose($fp);

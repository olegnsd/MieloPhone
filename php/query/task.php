...
#	    $sql = "select * from tasks where uid = :uid order by dateadd desc";
	    $sql = "select * from tasks where (".(!empty($callers) && count($callers) > 0? implode(" or ", $callers): "1").") ".(!$all? " AND send/total < 1 and state != 'stop'": "")." order by dateadd desc". ($all? " LIMIT 100": "");
#echo $sql;
#echo $sql;
#exit;
...
#exit;
	...
	function getTaskForProcessing($caller) {
	    $sql = "	SELECT
			  t.*
			FROM
			  tasks t,
...
*/
	...
	function getRingInfo($id) {
        ...
        $sql = "select count(*) as total from black_manually where phone LIKE '%$phone'";
	    $param[] = array(':phone', $phone, PDO::PARAM_STR);
            return $this->query( $sql, $param )->resultRow();
        }
...
#		$sql = "select count(*) as total from tasks t, tasks_base b where b.id = :pid and b.tid = t.id and t.uid = :uid";
	...
	function getIDapi($hash) {
        //$md5 = md5($_SESSION['CONF']['SALT']['SALT1'] . $client['id'] . $_SESSION['CONF']['SALT']['SALT2']);
	    //$sql = "select id from users where md5(" . $_SESSION['CONF']['SALT']['SALT1'] . "id" . $_SESSION['CONF']['SALT']['SALT2'] . ") = :hash";
	...
	function getMaxDateadd(){
        $param = [];
        $sql = "SELECT id, dateadd, state, total, send FROM tasks ORDER BY tasks.id DESC LIMIT 1";
	    $result = $this->query( $sql, $param )->resultRow();
	    return $result;
    }
    
    function updateMaxId($max_id, $user_id){
        $sql = "UPDATE notif_id SET task_id = :task_id, user_id = :user_id where id = 1";
        $param[] = array(':task_id', $max_id, PDO::PARAM_STR);
        $param[] = array(':user_id', intval($user_id), PDO::PARAM_INT);
	    $this->query( $sql, $param );
    }
        
    function getIdNotif() {
        $param = [];
        $sql = "SELECT * FROM notif_id where id=1";
	    $result = $this->query( $sql, $param )->resultRow();
        return $result;
    }
    ...
    function getNotifTask($id){
        $param[] = array(':id', $id, PDO::PARAM_STR);
        $sql = "SELECT dateadd, state, total, send FROM tasks where id = :id";
	    $result = $this->query( $sql, $param )->resultRow();
	    return $result;
    }
